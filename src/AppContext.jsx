    ...
    try {
      ...
      setSignals(prev => {
        newSignals.forEach(sig => {
          const isNew = !prev.find(s => s.id === sig.id)
          if (isNew && sig.signal.includes("STRONG")) {
            addNotification({
              title: "🔱 " + sig.signal + " — " + sig.name,
              message: "₹" + sig.price + " | " + sig.scanner,
              type: sig.signal.includes("BUY") ? "BUY" : "SELL",
            })
          }
        })
        return newSignals
      ...
      console.log("API cached:", err.message)
    }
  }, [])
